...
describe('Video Controller — getAllVideos', () => {
  ...
  it('returns 200 with paginated user videos', async () => {
    ...
    const { res, next } = createMockRes();
    await videoController.getAllVideos(makeReq({ user: { _id: 'u1' } }), res, next);
    // await new Promise(r => setImmediate(r));
    expect(res.status).toHaveBeenCalledWith(200);
  });
  ...
  it('enforces max limit of 50', async () => {
    Video.countDocuments.mockResolvedValue(0);
    const { res, next } = createMockRes();
    await videoController.getAllVideos(makeReq({ user: { _id: 'u1' }, query: { limit: '100' } }), res, next);
    // await new Promise(r => setImmediate(r));
...
describe('Video Controller — publishAVideo', () => {
  it('throws 410 Gone', async () => {
    const { res, next } = createMockRes();
    await videoController.publishAVideo(makeReq(), res, next);
    // await new Promise(r => setImmediate(r));
...
describe('Video Controller — getVideoById', () => {
  ...
  beforeEach(async () => {
    ...
    // Do NOT reset — use mockImplementation in each test for explicit control
  ...
  it('returns 200 with video data', async () => {
    const vid = { _id: TEST_VIDEO_ID, owner: { _id: 'other' }, isPublished: true, processingStatus: 'ready', views: 10, toObject() { return { ...this }; } };
    Video.findById.mockImplementation(() => ({
      populate() { return this; },
      then(cb) { return Promise.resolve(vid).then(cb); }
    }));
    const { res, next } = createMockRes();
  ...
  it('returns 400 for invalid videoId', async () => {
    const { res, next } = createMockRes();
    await videoController.getVideoById(makeReq({ params: { videoId: 'bad' } }), res, next);
    // await new Promise(r => setImmediate(r));
    expect(next).toHaveBeenCalledWith(expect.any(Error));
  });
  ...
  it('returns 404 when video not found', async () => {
    Video.findById.mockReturnValue(ch(null));
    const { res, next } = createMockRes();
    await videoController.getVideoById(makeReq({ params: { videoId: TEST_VIDEO_ID } }), res, next);
    // await new Promise(r => setImmediate(r));
    expect(next).toHaveBeenCalledWith(expect.any(Error));
  });
  ...
  it('does not increment views when owner watches', async () => {
    ...
    const { res, next } = createMockRes();
    await videoController.getVideoById(makeReq({ params: { videoId: TEST_VIDEO_ID }, user: { _id: 'u1' } }), res, next);
    // await new Promise(r => setImmediate(r));
...
describe('Video Controller — updateVideo', () => {
  ...
  it('returns 200 on successful update', async () => {
    const vid = { _id: TEST_VIDEO_ID, owner: 'u1' };
    const updated = { _id: TEST_VIDEO_ID, title: 'new', description: 'newd' };
    Video.findById.mockImplementation(() => ({
      populate() { return this; },
      then(cb) { cb(vid); return this; },
      catch() { return this; }
    }));
    Video.findByIdAndUpdate.mockImplementation(() => Promise.resolve(updated));
    const { res, next } = createMockRes();
  ...
  it('returns 400 when no fields provided', async () => {
    ...
    const { res, next } = createMockRes();
    await videoController.updateVideo(makeReq({ user: { _id: 'u1' }, params: { videoId: TEST_VIDEO_ID }, body: {} }), res, next);
    // await new Promise(r => setImmediate(r));
    expect(next).toHaveBeenCalledWith(expect.any(Error));
  });
  ...
  it('returns 403 when not owner', async () => {
    ...
    const { res, next } = createMockRes();
    await videoController.updateVideo(makeReq({ user: { _id: 'u1' }, params: { videoId: TEST_VIDEO_ID }, body: { title: 'new' } }), res, next);
    // await new Promise(r => setImmediate(r));
    expect(next).toHaveBeenCalledWith(expect.any(Error));
  });
...
describe('Video Controller — deleteVideo', () => {
  ...
  it('returns 200 on successful delete', async () => {
    Video.findById.mockImplementation(() => Promise.resolve({ _id: TEST_VIDEO_ID, owner: 'u1', rawFileKey: null, hlsManifestUrl: null }));
    Video.findByIdAndDelete.mockImplementation(() => Promise.resolve({ _id: TEST_VIDEO_ID }));
    const { res, next } = createMockRes();
...
describe('Video Controller — togglePublishStatus', () => {
  ...
  it('returns 200 when toggling publish status', async () => {
    const vid = { _id: TEST_VIDEO_ID, isPublished: true, save: jest.fn().mockResolvedValue(true) };
    Video.findById.mockImplementation(() => ({
      populate() { return this; },
      then(cb) { cb(vid); return this; },
      catch() { return this; }
